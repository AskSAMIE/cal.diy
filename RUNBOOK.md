...
- **Encryption in transit:** TLS on both cross-cloud hops; Cloud SQL `ssl_mode = ENCRYPTED_ONLY`; Memorystore `transit_encryption_mode = SERVER_AUTHENTICATION`. **Cloud SQL connectivity uses a `cloud-sql-proxy` sidecar** in the web/api services + migrate job — it performs fully-verified mTLS to Cloud SQL while cal connects over in-container `127.0.0.1:5432` (cal's Prisma JS adapter can't otherwise verify the Cloud SQL CA). `var.sql_ssl_mode` stays `ENCRYPTED_ONLY`; do NOT relax it. (`var.redis_transit_encryption = "DISABLED"` only if the cal Redis client can't present TLS, logged as residual risk.)
